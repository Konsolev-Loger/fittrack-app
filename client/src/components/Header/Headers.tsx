...
export const Header = () => {
 ...
 return <header className={styles.header}><div className={styles.inner}><Link to="/" aria-label="Setly — главная"><Brand/></Link><span className={styles.section}>Личный дневник</span><div className={styles.userBlock}><ThemeToggle/><Link to="/profile" title={user?.name || "Мой профиль"}>Профиль</Link><button type="button" disabled={isLoading} onClick={()=>void logout()}>Выйти</button></div></div>{error&&<p role="alert" className="shell">{error}</p>}</header>;
};
